...
def validate_execution(spec: ExecutionSpec, strict: bool = False) -> ValidationResult:
    ...
    param_names = set(spec.parameters.keys())
    for ref in refs:
        namespace, name = ref.split(".", 1)
        if namespace == "inputs" and name not in input_names:
            result.issues.append(
                ValidationIssue(
                    Severity.ERROR,
                    f"command.{ref}",
        ...
        elif namespace == "parameters" and name not in param_names:
            result.issues.append(
                ValidationIssue(
                    Severity.ERROR,
                    f"command.{ref}",
    ...
    elif spec.version.lower() == "latest":
        result.issues.append(
            ValidationIssue(
                Severity.WARNING,
                "version",
                "Version is 'latest' — use a specific version for reproducibility",
            )
        )
    ...
    # Check container for 'latest' tag (reproducibility concern)
    if spec.container and ":latest" in spec.container.lower():
        result.issues.append(
            ValidationIssue(
                Severity.WARNING,
                "container",
                "Container uses 'latest' tag — use a specific version for reproducibility",
            )
        )
...
def validate_interface(interface: InterfaceSpec, exec_spec: ExecutionSpec) -> ValidationResult:
    ...
    result = ValidationResult()
    all_fields = set(exec_spec.inputs.keys()) | set(exec_spec.parameters.keys())
